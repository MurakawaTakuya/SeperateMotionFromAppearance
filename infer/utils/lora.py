...
def _find_modules_v2(
    ...
    for ancestor in ancestors:
        for fullname, module in ancestor.named_modules():
            if any([isinstance(module, _class) for _class in search_class]):
                continue_flag = True
                if 'Transformer2DModel' in ancestor_class and ('attn1.to_k' in fullname or 'attn1.to_v' in fullname):
                    continue_flag = False
                if 'TransformerTemporalModel' in ancestor_class and ('attn1.to_q' in fullname or 'attn2.to_q' in fullname):
                    continue_flag = False
                if continue_flag:
                    continue
...
def _find_modules_v3(
    ...
    """
    只用temporal to_q训练temporal lora，不训练spatial lora
    Find all modules of a certain class (or union of classes) that are direct or
    indirect descendants of other modules of a certain class (or union of classes).
        ...
        for fullname, module in ancestor.named_modules():
            if any([isinstance(module, _class) for _class in search_class]):
                ...
                if 'Transformer2DModel' in ancestor_class and ('attn1' in fullname or 'ff' in fullname):
                    continue_flag = False
                if 'TransformerTemporalModel' in ancestor_class and ('attn1.to_k' in fullname or 'attn2.to_k' in fullname or 'attn1.to_out' in fullname or 'attn2.to_out' in fullname):
                    continue_flag = False
                if continue_flag:
                    continue
                # Find the direct parent if this is a descendant, not a child, of target
